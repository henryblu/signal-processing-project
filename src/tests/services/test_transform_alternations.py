...
class TestTransformAlternations(unittest.TestCase):
    ...
    def test_high_pitch_reduction(self):
        high_pitch_reduction_audio_data = (
            self.transform_alternations.high_pitch_reduction(self.transform)
        )
        ...
        high_pitch_reduction_audio_data = (
            self.transform_alternations.high_pitch_reduction(self.transform, 50)
        )
        self.assertEqual(high_pitch_reduction_audio_data.shape, self.transform.shape)
        self.assertTrue(
    ...
    def test_low_pitch_reduction(self):
        low_pitch_reduction_audio_data = (
            self.transform_alternations.low_pitch_reduction(self.transform)
        )
        ...
        low_pitch_reduction_audio_data = (
            self.transform_alternations.low_pitch_reduction(self.transform, 50)
        )
        self.assertEqual(low_pitch_reduction_audio_data.shape, self.transform.shape)
        self.assertTrue(
    ...
    def test_noise_reduction(self):
        noise_reduction_audio_data = self.transform_alternations.noise_reduction(
            self.transform
        )
